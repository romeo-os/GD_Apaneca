...
function initTable() {
    ...
    table = $('#table-content')
        ...
        .DataTable({
            ajax: "/Administrador/GetAreas",                                            //Url de donde tomara los datos
            //language: { url: "/assets/plugins/dataTable/lang-es.json" },
            language: { url: "/Content/es.json" },
            columns: [
                { data: "IdArea", sortable: false, searchable: false },
                { data: "Nombre" },                                                //Asigna el item  de la data obtenida a la segunda celda de la fila
                { data: "Funcion" },                                           //Asigna el item de la data obtenida a la tercera celda de la fila
                { data: "Encargado"},
                {
...
function bindButtons() {
    $('#table-content tbody tr td button').unbind('click').on('click', function (event) {
        ...
        if (action == 'edit') {
            getForm(obj.IdArea);                                                     //Pasa el id del elemento que se editará
        } else if (action == 'delete') {
            deleteRecord(obj.IdArea);                                                //Pasa el id del objeto que se eliminará
        }
    })
}
...
function getForm(id) {
    ...
    var toUrl = isEditing ? "/Administrador/EditarArea/" + id : "/Administrador/NuevaArea";
    title = isEditing ? "Editar Area" : "Crear Area";
...
function deleteRecord(id) {
    bootbox.confirm("¿Está seguro de eliminar el registro?", function (result) {
        if (result) {
            $.ajax({
                url: "/Administrador/EliminarArea/" + id,                                 //Url del API enpoint para eliminar el registro
                type: 'DELETE',
