...
function initTable() {
    ...
    table = $('#table-content')
        ...
        .DataTable({
            ajax: "/Administrador/GetRolesData",                                    //Url de donde tomara los datos
            language: { url: "/Content/es.json" },
            columns: [
                { data: "idRol", sortable: false, searchable: false },
                {
                    data: "rol",
                    render: function (data, type, row, meta) {
                        return '<a href="/Administrador/Permisos/' + row.idRol + '">' + data + '<a/>';
                    }
                },
                {
...
function bindButtons() {
    $('#table-content tbody tr td button').unbind('click').on('click', function (event) {
        ...
        if (action == 'edit') {
            getForm(obj.idRol);                                                         //Pasa el id del elemento que se editará
        } else if (action == 'delete') {
            deleteRecord(obj.idRol);                                                    //Pasa el id del objeto que se eliminará
        }
    })
}
...
function getForm(id) {
    ...
    var toUrl = isEditing ? "/Administrador/EditarRol/" + id : "/Administrador/NuevoRol";
...
function deleteRecord(id) {
    bootbox.confirm("¿Está seguro de eliminar el registro?", function (result) {
        if (result) {
            $.ajax({
                url: "/Administrador/EliminarRol/" + id,                                    //Url del API enpoint para eliminar el registro
                type: 'DELETE',
