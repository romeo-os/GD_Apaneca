...
function prepareButtons() {
    $('.button-create').click(function () {
        getForm(0);                                                                 // 0 para un crear un nuevo registro
        });
...
function initTable() {
    ...
    table = $('#table-content')
        ...
        .DataTable({
            ajax: "/Documentos/GetData",                                            //Url de donde tomara los datos
            ...
            columns: [
                { data: "IdCarpeta", sortable: false, searchable: false },
                {
                    ...
                    render: function (data, type, row, meta) {
                        return '<a href="/Documentos/ContenidoCarpetas/' + row.IdCarpeta + '" class="badge badge-success"><strong>' + data + '</strong><a/>';
                    }
                },                                                 //Asigna el item -nombre- de la data obtenida a la segunda celda de la fila
                { data: "Descripcion" },                                            //Asigna el item -descripcion- de la data obtenida a la tercera celda de la fila
                { data: "Fecha" },
                {
...
function bindButtons() {
    $('#table-content tbody tr td button').unbind('click').on('click', function (event) {
        ...
        if (action == 'edit') {
            getForm(obj.IdCarpeta);                                                     //Pasa el id del elemento que se editará
        } else if (action == 'delete') {
            deleteRecord(obj.IdCarpeta);                                                //Pasa el id del objeto que se eliminará
        }
    })
}
...
function getForm(id) {
    ...
    var toUrl = isEditing ? "/Documentos/ActualizarCarpeta/" + id : "/Documentos/CrearCarpeta";
    title = isEditing ? "Editar carpeta" : "Crear carpeta";
...
function deleteRecord(id) {
    bootbox.confirm("¿Está seguro de eliminar el registro?", function (result) {
        if (result) {
            $.ajax({
                url: "/Documentos/BorrarCarpeta/"+id,                                 //Url del API enpoint para eliminar el registro
                type: 'DELETE',
