...
function initTable() {
    ...
    table = $('#table-content')
        ...
        .DataTable({
            ajax: "/Mantenimientos/GetProcedimientos",                                            //Url de donde tomara los datos
            language: { url: "/Content/es.json" },
            columns: [
                { data: "idProcedimiento", sortable: false, searchable: false },
                { data: "Procedimiento" },                                                //Asigna el item  de la data obtenida a la segunda celda de la fila
                { data: "Duracion" },                                            //Asigna el item de la data obtenida a la tercera celda de la fila
                { data: "Prioridad" },
                {
...
function bindButtons() {
    $('#table-content tbody tr td button').unbind('click').on('click', function (event) {
        ...
        if (action == 'edit') {
            getForm(obj.IdProcedimiento);                                                     //Pasa el id del elemento que se editará
        } else if (action == 'delete') {
            deleteRecord(obj.IdProcedimiento);                                                //Pasa el id del objeto que se eliminará
        }
    })
}
...
function getForm(id) {
    ...
    var toUrl = isEditing ? "/Mantenimientos/EditarProceso/" + id : "/Mantenimientos/NuevoProceso";
    title = isEditing ? "Editar Procedimiento" : "Crear Procedimiento";
...
function startValidation() {
    $("#modal-form").validate({
        rules: {
            procedimiento: { required: true, minlength: 3, maxlength: 45 },
...
function deleteRecord(id) {
    bootbox.confirm("¿Está seguro de eliminar el registro?", function (result) {
        if (result) {
            $.ajax({
                url: "/Mantenimientos/EliminarProceso/" + id,                                 //Url del API enpoint para eliminar el registro
                type: 'DELETE',
